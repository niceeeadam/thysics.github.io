...
// This file draws and animates data returned by the Python physics models.
...
let mode="projectile", data=null, time=0, playing=false, lastFrame=0, version=0, debounce, animationId=0;
...
function buildControls(preset="custom") {
  ...
  $("preset").addEventListener("change",e=>{
    ...
    buildControls(selected); requestModel();
  });
  document.querySelectorAll(".number").forEach(input=>input.addEventListener("input",()=>{
    ...
    if(input.value==="" || !Number.isFinite(input.valueAsNumber) || input.valueAsNumber<Number(input.min) || input.valueAsNumber>Number(input.max)) {
      version++; clearTimeout(debounce); setError(`${input.getAttribute("aria-label")} must be between ${input.min} and ${input.max}.`); setBusy(true); return;
    }
  ...
  document.querySelectorAll("[data-op]").forEach(button=>button.addEventListener("click",()=>{
    ...
    $("preset").value="custom";requestModel();
  }));
...
function scheduleModel(){pause();version++;setBusy(true);clearTimeout(debounce);debounce=setTimeout(requestModel,100);}
async function requestModel(){
  clearTimeout(debounce);pause();setBusy(true);const ticket=++version;
  ...
  try{
    const response=await fetch(`/api/${mode}?${new URLSearchParams(state[mode])}`);
    const result=await response.json();if(ticket!==version)return;
    if(!response.ok)throw new Error(result.error||"The model could not be calculated.");
    data=result;time=0;setError();setBusy(false);updateContent();drawAll();
  }catch(error){if(ticket===version){setError(error.message==="Failed to fetch"?"The Python app is not responding. Keep its terminal open and refresh this page.":error.message);}}
}
function setMode(next){
  mode=next;version++;data=null;time=0;pause();setError();
...
$("reset").addEventListener("click",()=>{state[mode]={...DEFAULTS[mode]};buildControls(0);requestModel();});
